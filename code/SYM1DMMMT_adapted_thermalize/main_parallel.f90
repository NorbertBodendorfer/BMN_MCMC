...
program BFSS
  ...
  character(len=1000)::ntau_file
  real*8::wall_time
  real*8::traj_time_start
  real*8::traj_time_stop
  real*8::launch_time
  real*8::therm_sample_time_start
  real*8::therm_sample_time_stop
  real*8::avg_accept
  real*8::traj_time_stop_proper
  real*8::target_acceptance
  integer::accept_prev
  integer::nsample_therm
  integer::unit_ntau
  ...
  select case(narg)
      case(0)
          write(*,*)"ERROR: No input file name, walltime, and ntau file passed. Stopping program."
          stop
      case(1)
          write(*,*)"ERROR: Only one arguement passed. Stopping program."
      case(2)
          write(*,*)"ERROR: Only two arguements passed. Stopping program."
      case(3)
          ...
          call get_command_argument(3,ntau_file)           !stores the output file for the ntau 
          ...
          write(*,*)"Ntau file ",trim(adjustl(ntau_file))," being used."
      case default
          write(*,*)"ERROR: More than 3 argument passed. Pass only the input file. Stopping program."
          stop
  end select
  ...
  !Construct ntau outfile
  !ntau_file=trim("dt")//trim(data_output)  

  open(unit=unit_ntau,status='REPLACE',&
             &file=ntau_file,action='WRITE')
  ...
  therm_sample_time_start=MPI_Wtime()

  do while (itraj.LE.ntraj .AND. (traj_time_stop - launch_time + 3.0d0 * (traj_time_stop-traj_time_start) + 60.0d0 ).LE.wall_time )
     !write(*,*)"Start time is ",traj_time_start
     !write(*,*)"Stop time is ",traj_time_stop
     !call cpu_time(traj_time_start)
     ...
     !write(unit_ntau,*) itraj, ntau, dble(nacceptance-accept_prev)/dble(nsample_therm), (traj_time_stop - traj_time_start)


     !Compute average acceptance for the last 100 runs
     avg_accept=avg_accept+dble(nacceptance)/dble(ntrial)

     !Adjust ntau for efficient thermalization
     if(MOD(itraj,nsample_therm).EQ.0)then

             therm_sample_time_stop=MPI_Wtime()

             write(unit_ntau,*) itraj, ntau, dble(nacceptance-accept_prev)/dble(nsample_therm), &
                     &(therm_sample_time_stop-therm_sample_time_start), &
                     &dble(nacceptance-accept_prev)/dble(nsample_therm)/&
                     &(therm_sample_time_stop-therm_sample_time_start)


             therm_sample_time_start=therm_sample_time_stop

             if(dble(nacceptance-accept_prev).LT.target_acceptance * dble(nsample_therm))then
                     ntau=ntau+1
             else
                     ntau=ntau-1    
             end if
             ...
             dtau_xmat=0.1d0/dble(ntau)
       
             if(ngauge.EQ.0)then
                 dtau_alpha=0.14d0/dble(ntau)
             end if

             avg_accept=0.0d0
             accept_prev=nacceptance
     end if
