...
function create_pbs_file {
    ...
    echo "" >> $outfile
    echo "## TIME FOR SUBMIT: ${time}" >> $outfile
    echo "" >> $outfile
    echo "#SBATCH --clusters=qp4" >> $outfile
    # Set partition, 'qp4' or 'dev'
    echo "#SBATCH --partition qp4" >> $outfile
    # Set job name
    echo "#SBATCH --job-name=\"${jobname}\"" >> $outfile
    # Set working directory
    echo "#SBATCH --chdir=/home/bon39708/BMN/${pbs_folder}" >> $outfile
    # Nodes to be allocated
    echo "#SBATCH --nodes=${nNodes}" >> $outfile
    # Tasks (e.g MPI ranks) to run on each node
    echo "#SBATCH --ntasks-per-node=48" >> $outfile
    echo "" >> $outfile
    ...
    echo "srun -n ${nCores} ${exe_path_file} ${datfile_path}/${jobname}.dat ${time_cap}d0" >> $outfile
